...
export function emptyServiceValues() {
    const o = {};
...
function additionalsDisplaySum(sv) {
    return (sv.tv +
        sv.radio +
        sv.midias_sociais +
        sv.alertas_web +
...
export function collectSelectedMonitoringLabels(sections) {
    const acc = new Set();
 ...
 */
export function updateCalculations(input, prices) {
    let totalKeywords = 0;
    let totalVolume = 0;
    ...
    const destinatarios = Math.max(0, Math.floor(Number(input.operational.numDestinatarios) || 0));
    ...
    const valorAcrescimoFimDeSemana = factorWeekend > 1 ? afterWeekend - subtotalBeforeModifiers : 0;
    const valorImpactoAprovacaoAutomatica = priceAfterModifiersBeforeMonthlyBase - afterWeekend;
